...
const ModelDrawer = function (props: any) { 
  ...
  const handleSubPreview = async (file: UploadFile) => {
    if (!file.url && !file.preview) {
      file.preview = await getBase64(file.originFileObj as RcFile);
    }
  ...
  const handleSubChange: UploadProps['onChange'] = ({ fileList: newFileList }) => { 
...
<img alt="example" style={{ width: '100%' }} src={previewImage} />
  ...
  <Row gutter={16}>
      <Col span={24}>
          ...
          >
              <Upload
                  action="http://127.0.0.1:9000/img/subsave"
                              listType="picture"
                              accept='file'
                  fileList={fileList}
                  onPreview={handleSubPreview}
